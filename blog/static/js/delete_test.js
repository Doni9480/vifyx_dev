...
async function delete_post(e) {
    ...
    url = window.location.protocol + '//' + window.location.host + '/api/v1/tests/' + test_id + '/delete/';
    ...
    if (response.ok) {
            window.location.replace(window.location.protocol + '//' + window.location.host + '/tests/');
    } else {
        alert('Backend error');
    }
}